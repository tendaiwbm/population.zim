...
class Distribution(View):
    ...
    def prepare_distribution(self,query):
        ...
        step = 100
        for i in range(0,len(queryResult[0]),step):
            geom = (pair[1] for pair in queryResult[i:i+step])
            RESPONSE = {
                        ...
                        "grain": "ward",
                        "values": [log10(pair[0]) if pair[0] > 0 else pair[0] for pair in queryResult[i+step]],
                       }
    ...
    def get(self,request):
        ...
        if distro_validator(request.GET):
            ...
            return self.prepare_distribution(query)
