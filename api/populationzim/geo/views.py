...
def fetch_distribution(request):
    if distro_validator(request.GET):
        ...
        if request.GET['grain'] == "ward":
            query = f"""select {request.GET['sex']}_population_density_{request.GET['year']},st_astext(shape) from prelim.{request.GET['grain']}"""
            ...
            query = f"""
                        SELECT sum(b.{request.GET['sex']}_population_density_{request.GET['year']}),st_astext(shape)
            ...
            query = """""".join([query,subQuery, f""") as b ON a.{request.GET['admin']}_name = b.{request.GET['admin']}_name GROUP BY a.shape"""])
